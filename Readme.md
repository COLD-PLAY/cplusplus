...
```
volatile：volatile 关键字是一种类型修饰符，用它声明的类型变量表示可以被某些编译器未知的因素更改，比如：操作系统、硬件或者其它线程等。遇到这个关键字声明的变量，编译器对访问该变量的代码就**不再进行优化**，从而可以提供对特殊地址的稳定访问。突出一个**不优化**。volatile 可以保证对特殊地址的稳定访问，比方说我定义了一个变量a，编译器将它优化到一个寄存器里面放着，同时我用内联汇编给这个寄存器改了，后面用到这个变量时也不去内存里面重新拿值，而是直接从这个寄存器里面拿就不对劲了
